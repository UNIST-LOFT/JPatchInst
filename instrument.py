...
def instrument(project:str,path:str):
    result=subprocess.run(['java','-cp','/root/project/APR-instrumenter/build/libs/APR-instrumenter.jar',
                        'kr.ac.unist.apr.InstrumentationMain','/root/project/APR-instrumenter/build/classes/java/main',
                        f'{path}/{project}/target/classes'])
    if result.returncode != 0:
        print(f'Instrumentation failed for {project}')
        exit(1)

    result2=subprocess.run(['cp','~/project/APR-instrumenter/build/classes/java/main/kr/ac/unist/apr/GlobalStates.class',
